 ...
 *
 * Argument parsing lives here rather than in the command layer so the two Feishu
 * routes stay independently runnable (`collect feishu` = open API, this one = MCP).
 */
...
export async function runCollectCli(argv, io = {}) {
  ...
  let config;
  try {
    const credential = loadCredential({ env: process.env });
    config = { ...credential.values, credential_source: credential.source };
  } catch (error) {
    err(`collect feishu --mode mcp: ${error.message}`);
    return {
      ok: false,
      exitCode: 1,
      receipt: {
        ...
        unavailable: [{ channel: CHANNEL, reason: error.message, remediation: ["write ~/.distilly/feishu_config.json with app_id and app_secret"] }],
      },
    };
  }
  ...
  try {
    if (options.url) {
      const mapped = toolForUrl(options.url);
      tool = mapped.tool;
      toolArgs = mapped.arguments;
      target = options.url;
    } else {
      tool = "get_chat_messages";
      toolArgs = { chat_id: options.chatId, page_size: Math.min(Number(options.limit) || 50, 50) };
    ...
    return {
      ...
      receipt: {
        ...
        anchors: { total: 0, cited: 0 },
        errors: [error.message],
        unavailable: [{ channel: CHANNEL, reason: error.message, remediation: ["pass a Feishu doc/wiki/sheet URL, or --chat-id"] }],
      },
    };
  }

  const transport = io.transport ?? spawnMcpTransport();
  ...
  if (!options.json) {
    if (result.ok) {
      out(`collect feishu (mcp): ${result.receipt.messages} message(s) via ${tool}, ${result.receipt.anchors.total} anchor(s)`);
    ...
    for (const item of result.receipt.unavailable ?? []) err(`  unavailable: ${item.reason}`);
  }
  return result;
}
