...
test("a person Skill carries its evidence directories into the install", () => {
  const root = tempDir();
  try {
    const skillDir = createSkill(
      join(root, "skills", "colleague"),
      "lin-gong",
      { character: "colleague", name: "林工" },
      "Work body",
      "Persona body",
    );
    // The v2 layout a harvested Skill has on disk.
    mkdirSync(join(skillDir, "knowledge", "text"), { recursive: true });
    writeFileSync(join(skillDir, "knowledge", "text", "subtitle.md"), "k0001 正文\n", "utf8");
    writeFileSync(join(skillDir, "knowledge", "index.json"), "[]\n", "utf8");
    mkdirSync(join(skillDir, "evidence", "derived"), { recursive: true });
    writeFileSync(join(skillDir, "evidence", "derived", "stats.json"), "{}\n", "utf8");
    mkdirSync(join(skillDir, "views"), { recursive: true });
    writeFileSync(join(skillDir, "views", "lin-gong.view.json"), "{}\n", "utf8");

    const skillsDir = join(root, "host-skills");
    const result = installGeneratedSkill({ skillDir, skillsDir, host: "codex" });

    for (const relativePath of [
      "knowledge/text/subtitle.md",
      "knowledge/index.json",
      "evidence/derived/stats.json",
      "views/lin-gong.view.json",
    ]) {
      assert.ok(existsSync(join(result.skill_dir, relativePath)), `${relativePath} must travel with the Skill`);
    }

    const record = JSON.parse(readFileSync(join(result.skill_dir, ".distilly-install.json"), "utf8"));
    assert.deepEqual(record.carried, ["knowledge/raw", "knowledge/text", "evidence", "views"].filter((d) => existsSync(join(result.skill_dir, d))));
    assert.ok(record.carried.includes("knowledge/text"));
    assert.ok(record.carried.includes("evidence"));
    assert.ok(record.carried.includes("views"));
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
