...
## 7. 后续更正（2026-09-15）：两条判据当时是"假绿"

这一版审计自己踩过的两个坑，在第 3 节只记了一条（`--merged`），而且记的修法（"改为按
合并提交统计"）并没有真正落地；另一条当时没被发现。两条都在 2026-09-15 重写：

| 行 | 当时的样子 | 为什么是假绿 | 现在 |
| --- | --- | --- | --- |
| 11「每个已合并分支都有 PR 证据文档」 | `git branch --merged HEAD` | 19 条 `ds/*` 特性分支**一条都没合并**，匹配到 0 条，`withoutDoc.length === 0` 恒真——"每个 PR 都有测试/前后对比/回滚文档"这件事从未被验证 | 检查**所有本地 `ds/NN-*` 分支**（合并与否都算）；单 ref 检出（CI）时记为**声明缺口**并说明，不假绿也不误红 |
| 12「截图不入库」 | `["dst-evidence","evidence/renders"]` 上取 `.some(Boolean)` | 探的是 `dst-evidence`（不带斜杠），目录模式 `dst-evidence/` 匹配不上不存在的目录 → 返回 false；只要另一个探针为真这一行就绿，证据串里还印着 false | 探目录**里面的路径**，并要求**每一个**探针都过（`every`） |

改完之后这条审计第一次真的会红：CI 的 checkout 只取一个 ref，第 11 行在那里
"无从检查"——于是它记成缺口（可见），而不是像以前那样悄悄变真。

配套的一次教训写在 `docs/v2/STATUS.md` 第 7 节：**绿必须来自判据，不能来自"没有对象可查"。**
